...
export async function startPreviewServer({
  ...
  let publishLatched = false;
  let pendingAbortTimer = null;
  ...
  function parseStreamLine(line, requestId) {
    ...
    // Flip session state once claude confirms the session is established.
    if (obj.type === "system" && obj.subtype === "init" && isFirstChat) {
      isFirstChat = false;
    }
  ...
  const server = http.createServer((req, res) => {
    ...
    if (req.method === "GET" && url.pathname === "/__preview__/overlay.js") {
      ...
      const cfgJson = JSON.stringify({ token, port: server.address().port, mode, claudeAvailable });
      const body = raw.replace("__CLAWPAGE_PREVIEW_CONFIG__", cfgJson);
    ...
    if (req.method === "GET" && url.pathname === "/__preview__/events") {
      ...
      sseClients.add(res);
      // SSE reconnect cancels any pending /quit-triggered abort.
      if (pendingAbortTimer) { clearTimeout(pendingAbortTimer); pendingAbortTimer = null; }
    ...
    if (req.method === "POST" && url.pathname === "/__preview__/chat") {
      ...
      if (!claudeAvailable) { res.writeHead(503, { "Content-Type": "application/json" });
        res.end(JSON.stringify({ error: "claude_not_available" })); return; }
    ...
    if (req.method === "POST" && url.pathname === "/__preview__/quit") {
      ...
      // If publish has already succeeded, /quit is a no-op (browser is just
      // navigating away to the live URL; this is not a user abort).
      if (publishLatched) return;
      // Otherwise, schedule abort with a grace window for SSE reconnect.
      if (pendingAbortTimer) clearTimeout(pendingAbortTimer);
      pendingAbortTimer = setTimeout(() => {
        pendingAbortTimer = null;
        if (!publishLatched && sseClients.size === 0) resolveAborted("quit");
      }, 5000);
      return;
    }
