...
import {
  loadKeys, createPage, updatePage, buildFailureResult,
  buildSuccessPayload,
} from "./publish.mjs";
...
async function main() {
  ...
  // Soft-check claude binary. If missing, preview still runs in publish-only
  // mode (chat FAB is hidden client-side; /chat returns 503 server-side).
  ...
  const claudeAvailable = !probe.error && probe.status === 0;
  if (!claudeAvailable) {
    console.error(`[clawpage preview] claude binary not detected (tried "${claudeBinary} --version") — chat-edit disabled. Publish still works. Install Claude Code: https://docs.claude.com/claude-code`);
  }
  ...
  const publishHandler = async () => {
    ...
    try {
      ...
      const ttlMsApplied = pageId
        ? (typeof ttlMs === "number" ? ttlMs : null)
        : (ttlMs === undefined ? DEFAULT_CREATE_TTL_MS : ttlMs);
      const successPayload = buildSuccessPayload({
        data, pageId, isUpdate: !!pageId, pagecode, ttlMsApplied,
      });
      const liveUrl = successPayload.publicUrl || successPayload.accessUrl || successPayload.rootUrl;
