(function () {
  ...
  const STYLE = `
    ...
    .fab-tip { position: fixed; right: 90px; padding: 10px 14px;
               background: rgba(17,24,39,.94); color: white; border-radius: 10px;
               font: 12px/1.5 -apple-system,system-ui,sans-serif; max-width: 230px;
               z-index: 2147483601; box-shadow: 0 8px 20px rgba(0,0,0,0.18);
               opacity: 0; transform: translateX(8px);
               transition: opacity .25s ease, transform .25s ease; }
    .fab-tip.show { opacity: 1; transform: translateX(0); }
    .fab-tip::after { content: ""; position: absolute; right: -7px; top: 50%;
                      transform: translateY(-50%);
                      border: 7px solid transparent; border-left-color: rgba(17,24,39,.94); }
    .fab-tip-publish { bottom: 30px; }
    .fab-tip-chat { bottom: 100px; }
    .fab-tip .x { float: right; cursor: pointer; opacity: 0.6; padding: 0 0 0 8px;
                  margin-left: 4px; }
  ...
  // ---------- onboarding bubbles (one-time per browser) ----------
  ...
  function showOnboardingBubbles() {
    ...
    if (cfg.claudeAvailable !== false) {
      tips.push({
        cls: "fab-tip-chat",
        text: "Chat with Claude to refine this page — type any change, watch it apply live.",
        anchor: "__clawpage_chat_fab__",
      });
    }
    tips.push({
      cls: "fab-tip-publish",
      text: cfg.mode === "update"
        ? "Click here to republish — your local edits go live."
        : "Click here when you're happy — publishes to clawpage.ai and redirects you.",
      anchor: "__clawpage_publish_fab__",
    });

    const created = [];
    for (const t of tips) {
      const tip = document.createElement("div");
      tip.className = "fab-tip " + t.cls;
      tip.innerHTML = `<span class="x" aria-label="dismiss">×</span><span class="msg"></span>`;
      tip.querySelector(".msg").textContent = t.text;
      shadow.appendChild(tip);
      requestAnimationFrame(() => tip.classList.add("show"));
      const dismiss = () => { tip.classList.remove("show"); setTimeout(() => tip.remove(), 250); };
      tip.querySelector(".x").addEventListener("click", (e) => { e.stopPropagation(); dismiss(); });
      const fab = shadow.getElementById(t.anchor);
      if (fab) fab.addEventListener("click", dismiss, { once: true });
      created.push({ tip, dismiss });
    }
    // Auto-fade after 12s.
    setTimeout(() => { for (const c of created) c.dismiss(); }, 12000);
    try { localStorage.setItem(ONBOARD_KEY, "1"); } catch {}
  }
  // Defer slightly so the FABs are painted first.
  setTimeout(showOnboardingBubbles, 400);
})();
