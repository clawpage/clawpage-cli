...
export function buildSuccessPayload({ data, pageId, isUpdate, pagecode, ttlMsApplied }) {
  ...
  const pageNameFinal = page.pageName ?? null;
  ...
  const summary = buildSuccessSummary({
    mode, pageId: page.pageId || pageId, publicUrl, rootUrl, accessUrl,
  });
  return {
    ok: true,
    mode,
...
async function main() {
  ...
  const result = buildSuccessPayload({
    data, pageId, isUpdate, pagecode, ttlMsApplied,
  });
  result.htmlPath = outputHtml; // publish.mjs-only field
