...
function usage() {
  ...
  --delete <pageId> --yes      permanently delete a page (--yes is required)
...
async function main() {
  ...
  try {
    ...
    if (typeof args.delete === "string") {
      if (args.yes !== true) {
        throw new Error("refusing to delete without --yes (deletion is permanent and cannot be undone)");
      }
      await call(`${base}/${encodeURIComponent(args.delete)}`, { method: "DELETE", token: keys.token });
      console.log(JSON.stringify({ deleted: args.delete, dataFetchedAt }, null, 2));
      return;
    }
    usage();
    process.exit(1);
